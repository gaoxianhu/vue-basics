...
const { CleanWebpackPlugin } = require('clean-webpack-plugin');//每一次打包自动清理dist
...
module.exports = {
    ...
    module: {
        rules: [
            //解析ES6语法
            {
                ...
                use: {
                  loader: 'babel-loader',//本身并不能解析es6语法，它依赖的是babel/preset-env去解析的
            ...
            {
                test: /\.css$/,
                use: [ 'vue-style-loader', 'css-loader' ]//使用loader的时候有顺序，从后往前走
            },
    ...
    devServer: {
        ...
        proxy: {
          'api': {//这个/api其实是为了告诉代理，以后什么样的请求，需要给我的代理转发
            target:'http://localhost:4000',
            //转发的目标地址，不需要路径，因为转发的时候会把发送请求的路径默认拼到目标后面
            ...
            //这一行的作用就是把/api去掉，就是真正的目标地址
            changeOrigin:true, //支持跨域，如果协议/主机也不相同，必须加上
          }
        },
        historyApiFallback:true,//任意的404响应都会被替代为index.html 备胎
    },
